...
const Register = ({ navigation }) => {
    ...
    const handleRegister = async () => {
        ...
        try {
            ...
            Alert.alert("Lỗi", "Đăng ký thất bại! Tên đăng nhập hoặc email có thể đã tồn tại.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <ScrollView style={[MyStyles.container, { paddingHorizontal: 16 }]}>
            ...
            <View style={{ alignItems: 'center', marginBottom: 20 }}>
                <TouchableOpacity onPress={pickImage}>
                    {avatar ? (
                        ...
                        <Avatar.Icon size={90} icon="camera" backgroundColor="#e3f2fd" color="#005b9f" />
                    )}
                </TouchableOpacity>
            ...
            <TextInput label="Ngày sinh (YYYY-MM-DD)" value={dob} onChangeText={setDob} placeholder="..." style={styles.input} mode="outlined" />
            
            <Text style={styles.genderLabel}>Giới tính:</Text>
            <RadioButton.Group onValueChange={newValue => setGender(newValue)} value={gender}>
                <View style={styles.genderRow}>
                    <View style={styles.radioItem}>
                        <RadioButton value="MALE" color="#005b9f" />
                        <Text>Nam</Text>
                    </View>
                    <View style={styles.radioItem}>
                        <RadioButton value="FEMALE" color="#005b9f" />
                        <Text>Nữ</Text>
                    </View>
                    <View style={styles.radioItem}>
                        <RadioButton value="OTHER" color="#005b9f" />
                        <Text>Khác</Text>
                    </View>
                </View>
            </RadioButton.Group>
            ...
            {loading ? (
                ...
                <Button mode="contained" onPress={handleRegister} style={{ marginTop: 20, padding: 4 }} buttonColor="green">
                    ĐĂNG KÝ
                </Button>
            )}

            <Button mode="text" onPress={() => navigation.navigate('Login')} style={{ marginTop: 10, marginBottom: 40 }}>
                Quay lại Đăng nhập
            </Button>
        </ScrollView>
    );
